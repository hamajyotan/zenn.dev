...
- Rails アプリケーションを Github Codespaces で動かすには `--devcontainer` オプションをつけた `rails new` の構成でおおよそ動く
- ただし、 `localhost` ではないドメイン名でのアクセスなので、 `config.hosts` の調整を要する
    - (この点は codespace でなくても同様)
- codespace 独自の問題として、HTTP Origin ヘッダによるサイトのオリジンチェックにひっかかるので  `config.action_controller.forgery_protection_origin_check` を `false` に指定する必要がある
    - (2025/04/23 時点の構成)

# 上記結論以外で伝えたいこと

ブラウザ上で開発環境を気軽に体験できるサンプルアプリケーションの環境として Github Codespaces は良い選択だと思います。当然そのまま実行もできます。
Rails で複数の ActiveRecord モデルを内包し、あたかもそれらがひとつのモデルであるように振る舞うことことを提供する gem として [active_record_compose](https://github.com/hamajyotan/active_record_compose) を作成しています。
これを、実際に試すための環境を用意しているのでよければさわってみてください。
...
setup を終えたら、コンソールから `rails s` で起動できると思います。
...
既定で、ポートの表示範囲に Private が設定されているのですがこの場合は GitHub にログイン状態であれば直接ブラウザからアクセスできます。
...
レポートの解決の仕方によっては、上記ワークアラウンドは将来不要になるかもしれません。
:::
...
が、実はまだ GET リクエストだけしか処理できず、 POST リクエストはエラーになります。
...
本来は既定値を可能な限り有効な状態で動作させたいところですが、 GitHub Codespaces で動作させる場合にこの設定値を無効化するように調整しました。
...
- ブラウザ上で開発環境を気軽に体験できるサンプルアプリケーションの環境として Github Codespaces は良い選択だと思います。当然そのまま実行もできます。
- Rails アプリケーションを動かすためには主に以下の作業が必要でした。
    - `rails new` に `--devcontainer` オプションをつけた状態をベースに
    - localhost での動作ではないので `config.hosts` を調整する
    - `forgery_protection_origin_check` を無効化 (これは将来は不要になるかもしれません)
