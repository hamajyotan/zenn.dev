...
- codespace でなくても必要な一般的な注意点
  - `localhost` ではないドメイン名でのアクセスなので、 `config.hosts` の調整を要する
- codespace 独自の問題 (2025/04/27 時点)
  ...
  - 初期状態で GitHub Codespaces のポートフォワーディングが効かないので以下いずれかのワークアラウンドを要する
    - ポートの公開範囲を、初期値の Private 状態から一旦 Public して再び Private に戻すとなぜか Private (自分だけアクセス可) が機能する
    - docker image を microsoft 提供のものに差し替える
    - `.devcontainer/devcontainer.json` の設定、 `forwardPorts` を無効化する
...
setup を終えたら、コンソールから `rails s` で起動できると思います。。。 が、まだアクセスできません (後述)
