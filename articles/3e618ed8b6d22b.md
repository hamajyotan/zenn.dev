...
* 例えば、モデル A とモデル B をカプセル化するモデルを設計する場合、モデル A に `attribute_a`、モデル B に `attribute_b` があるとき、それぞれの属性に対して `model.assign_attributes(attribute_a: 'foo', attribute_b: 'bar')` のように透過的にアクセスできること。
...
上記で定義されたモデルを扱うコントローラーとビューは、以下のようになります。このコードは、`User` と `Profile` の関係についての知識を必要とせず、コントローラーやビューからモデルの構造を理解する必要もありません。まるで scaffold が生成したような構成です。
